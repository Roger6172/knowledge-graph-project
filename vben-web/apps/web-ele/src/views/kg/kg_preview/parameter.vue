...
// 文档 ID 列表
const docIdOptions = ref<Array<{ label: string; value: string }>>([
  { label: '全部文档', value: '' }
])
...
async function fetchDocuments() {
  ...
  try {
    const response = await baseRequestClient.get<any>('/kg/documents')
    ...
    if (result?.success && Array.isArray(result.data?.documents)) {
      docIdOptions.value = [
        ...
        ...result.data.documents.map((doc: any) => ({
          label: doc.title || doc.doc_id || doc.id,
          value: doc.doc_id || doc.id
        }))
      ]
      console.log('文档选项:', docIdOptions.value)
    } else {
      console.warn('文档列表响应格式不正确:', result)
    }
  } catch (error) {
...
<template>
  <div class="w-80 h-full overflow-y-auto p-4 flex flex-col text-gray-200 bg-gray-900/80 backdrop-blur-sm">
    ...
    <!-- 文档筛选 -->
    <div class="mb-6 group">
      <label class="text-gray-400 text-xs font-medium mb-2 block uppercase tracking-wider group-focus-within:text-cyan-400 transition-colors">文档筛选</label>
      <ElSelect
        v-model="localState.selectedDocId"
        class="style-select w-full"
        popper-class="style-select-dropdown"
        :teleported="false"
        :loading="loadingDocs"

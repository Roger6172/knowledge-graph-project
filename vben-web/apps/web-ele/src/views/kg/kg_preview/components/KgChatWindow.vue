  ...
  <div class="kg-chat-window flex flex-col h-full bg-gray-900/80 backdrop-blur-sm text-gray-200">
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <button 
          class="flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-gray-800 transition-colors group"
          @click="() => { showSessionList = true; loadSessions(); }"
        >
          <span class="font-semibold text-base tracking-wide text-gray-200 group-hover:text-cyan-400 transition-colors">
            {{ currentSessionName }}
          </span>
          <el-icon class="text-gray-500 group-hover:text-cyan-400 transition-colors"><ArrowDown /></el-icon>
        </button>
      </div>
      <div class="flex items-center gap-2">
        ...
        >
          ...
          >
            <div class="truncate">{{ doc.title || doc.doc_id }}</div>
          </ElOption>
        </ElSelect>
...
const canSend = computed(() => inputText.value.trim() && !isLoading.value && serviceStatus.value === 'online');
...
async function handleSend() {
  ...
  try {
    // 尝试调用 GraphRAG 混合搜索（带对话历史）
    let result;
    try {
      result = await hybridSearch(question, selectedStrategy.value, 5, chatHistory, currentSessionId.value, selectedDocId.value || undefined);
    } catch (hybridError) {
      // 如果混合搜索失败，回退到简单问答
      console.warn('Hybrid search failed, falling back to simple QA:', hybridError);
      const simpleResult = await simpleQA(question);
      result = {
        success: simpleResult.success,
        answer: simpleResult.answer,
        strategy_used: 'local' as SearchStrategy,
        local_result: {
          success: true,
          answer: simpleResult.answer,
          entities: [],
          relations: [],
          chunks: simpleResult.raw_docs?.map((doc: any, idx: number) => ({
            text: doc.text || doc.content || String(doc),
            score: doc.score,
            neo_id: idx,
          })) || [],
          community_context: [],
    ...
    if (lastMessage && lastMessage.role === 'assistant') {
      ...
      lastMessage.strategy = result.strategy_used;

      // 提取附加信息
      if (result.local_result) {
        lastMessage.entities = result.local_result.entities;
        lastMessage.relations = result.local_result.relations;
        lastMessage.chunks = result.local_result.chunks;
      ...
      if (result.global_result) {
        lastMessage.communities_used = result.global_result.communities_used;
      }

      // 流式输出
      await streamText(lastMessage, result.answer || '未能生成回答');
    }
  } catch (error: any) {

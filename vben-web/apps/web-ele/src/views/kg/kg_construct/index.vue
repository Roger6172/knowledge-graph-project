...
const docOptions = ref<Array<{ label: string; value: string }>>([{ label: '全部文档', value: '' }]);
...
const fetchDocuments = async () => {
  ...
  try {
    const res = await baseRequestClient.get<any>('/kg/documents');
...
const initGraph = async () => {
  ...
  try {
    // 传入文档筛选参数
    const res = await getVisualizeData(200, selectedDocId.value || undefined);
    const data = res.data || res;
...
const handleTextBuild = async () => {
  ...
  try {
    ...
    const uploadRes = await uploadDocument(form.text, form.docId, form.title);
    ...
    addLog('正在执行社区检测...');
    addLog('⏳ 这可能需要 1-5 分钟，请勿关闭页面');
    const detectRes = await detectCommunities();
    ...
    addLog('正在生成社区报告...');
    addLog('⏳ 这可能需要 1-5 分钟，请勿关闭页面');
    const reportRes = await generateCommunityReports();

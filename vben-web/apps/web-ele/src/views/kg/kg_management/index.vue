...
const loadDocuments = async () => {
  try {
    ...
    // 假设后端返回结构为 { success: true, data: { documents: [...] } } 或直接返回数组
    const docsList = data.data?.documents || data.documents || [];
    
    if (Array.isArray(docsList)) {
      documents.value = docsList.map((doc: any) => ({
...
const loadCommunities = async () => {
  ...
  try {
    const res = await getCommunities(selectedDocId.value);
    const data = res.data || res;
...
const initGraph = async () => {
  ...
  try {
    const res = await getVisualizeData(200, selectedDocId.value || undefined, selectedCommunityId.value);
    const data = res.data || res;
...
const handleDeleteDoc = async () => {
  ...
  try {
    ...
    loading.value = true;
    const res = await deleteDocument(selectedDocId.value);
    const data = res.data || res;
...
const handleExportDoc = async () => {
  ...
  try {
    loading.value = true;
    const res = await exportDocument(selectedDocId.value);
    const data = res.data || res;
    ...
    if (data.success) {
      ...
      link.href = url;
      link.download = `graph_export_${selectedDocId.value}.json`;
        ...
        <div class="flex-none bg-gray-800/40 rounded-xl p-4 border border-gray-700/50 backdrop-blur-sm">
          ...
          <div class="space-y-4">
            <div class="space-y-1.5">
              <label class="text-xs font-medium text-gray-500 ml-1">选择文档</label>
              <el-select
            ...
            <div class="space-y-1.5" :class="{ 'opacity-50 pointer-events-none': !selectedDocId }">
